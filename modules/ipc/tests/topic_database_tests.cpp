...
TEST(ZenohTests, TopicDatabase) {
  ...
  const auto publisher_topic =
      createTopicConfig(fmt::format("test_publisher/{}", random::random<std::string>(mt, 10, false, true)));
  auto publisher = Publisher<types::DummyType>{ session, publisher_topic };

  const auto subscriber_topic =
      createTopicConfig(fmt::format("test_subscriber/{}", random::random<std::string>(mt, 10, false, true)));
  auto subscriber =
      Subscriber<types::DummyType>{ session, subscriber_topic, [](const auto&, const auto&) {} };

  const auto service_topic =
      createTopicConfig(fmt::format("test_service/{}", random::random<std::string>(mt, 10, false, true)));
  ...
  const auto service_string_topic = createTopicConfig(
      fmt::format("test_service_string/{}", random::random<std::string>(mt, 10, false, true)));
